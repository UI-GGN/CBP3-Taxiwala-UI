  ...
  const Step1 = () => {
    ...
    return <>      
      ...
      {(alignment=="Ad-Hoc")&&<div>        
        <Alert severity="info">Ad-Hoc requests are request on demand basis for the specific day. These are to be made 1 hour before atleast.</Alert>
        <br/>
        ...
        />
        <br />     
        {(cabtype=="pick"||cabtype=="both")&&<Dropdown
          label="Check In Time"
          handleChange={(value: string) => setCheckintime(value)}
          value={checkintime}
          dropdownvalues={dropdownvalues}
        />}     
        {(cabtype=="drop"||cabtype=="both")&&<Dropdown 
      ...
      {alignment=="Regular"&&<div>        
        <Alert severity="info">Regular requests are when you plan to come for few days to office. 
        These are to be made 1 day before by 10pm.</Alert>
        <br/>
        <div>
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <DemoContainer components={['DatePicker']}> 
              <DatePicker label="Select start date" />
              <DatePicker label="Select end date" disabled={noEndDateNeeded}/>
            </DemoContainer>
        ...
        <FormControlLabel
          control={
            <Checkbox checked={noEndDateNeeded} onChange={(e) => setNoEndDateNeeded(e.target.checked)} name="isEndDate" />
          }
